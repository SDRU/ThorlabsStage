...
# OWN DEFINITIONS SHUTTER
TShutterCycleParams = collections.namedtuple("TShutterCycleParams", ["on_time", "off_time", "num_cycles"])
...
class KinesisDevice(BasicKinesisDevice,IStage):
    ...
    ############### SHUTTER FUNCTIONS
    def set_cycle_params(self,on_time=1000, off_time=1000, num_cycles=5):
        """
        Set parameters for shutter cycles

        Parameters
        ----------
        on_time : in ms
        off_time : in ms
        num_cycles : integer of course

        """
        message = struct.pack("<HLLL",1,int(on_time),int(off_time),int(num_cycles))
        self.send_comm(0x04C3,param1=0x0E,param2=0x00,source=0x01,dest=0x50)
        self.send_comm_data(0x04C3,data=message,source=0x01,dest=0x50)
        return self.get_cycle_params()
        
        
    def get_cycle_params(self):
        """
        Parameters of shutter cycle
        """
        params=self.query(0x04C4,param1=0x01,param2=0x00,source=0x01,dest=0x50).data
        on_time, off_time, num_cycles = struct.unpack("<LLL",params[2:14])
        return TShutterCycleParams(on_time, off_time, num_cycles)
    
    def shutter_open(self):
        self.send_comm(0x04CB,param1=0x01,param2=0x01,source=0x01,dest=0x50)
        
    def shutter_close(self):
        self.send_comm(0x04CB,param1=0x01,param2=0x02,source=0x01,dest=0x50)
    ...
    def set_operating_mode(self, mode=1):
        ...
        """
        self.send_comm(0x04C0,param1=0x01,param2=mode,source=0x01,dest=0x50)
...
class KinesisMotor(KinesisDevice):
    ...
    def set_trigger_params(self,tr1_mode=None,tr1_polarity=None,tr2_mode=None,tr2_polarity=None):
        ...
        self.send_comm(0x0523,param1=0x0C,param2=0x00,source=0x01,dest=0x50)
        self.send_comm_data(0x0523,data=message,source=0x01,dest=0x50)
        return self.get_trigger_params()


    def set_triggerin_abs_move(self,position=0):
        """  
        Moves to an absolute position after receiving a trigger
        ...
        self.send_comm(0x0450,param1=0x06,param2=0x00,source=0x01,dest=0x50)
        self.send_comm_data(0x0450,data=message,source=0x01,dest=0x50)
        
    def set_triggerin_rel_move(self,move_step=1000):
        """  
        Moves to a relative position after receiving a trigger
        
        Parameters
        ----------
        move_step : in encoder steps
        """
        message=struct.pack("<Hl",1,move_step)
        self.send_comm(0x0445,param1=0x06,param2=0x00,source=0x01,dest=0x50)
        self.send_comm_data(0x0445,data=message,source=0x01,dest=0x50)
    ...
    def set_triggerout_position_params(self,start_pos_fw=None,interval_fw=None,num_pulses_fw=None,start_pos_rev=None,interval_rev=None,num_pulses_rev=None,pulse_width=None,num_cycles=None):
        ...
        self.send_comm(0x0526,param1=0x22,param2=0x00,source=0x01,dest=0x50)
        self.send_comm_data(0x0526,data=message,source=0x01,dest=0x50)  
        return self.get_triggerout_position_params()
    
    def get_motor_output_params(self):
        C = 327.68
        
        params=self.query(0x04DB,param1=0x01,param2=0x00,source=0x01,dest=0x50).data
        cont_curr, energy_lim, motor_lim, motor_bias=struct.unpack("<HHHH",params[2:10])
            
        return TMotorOutputParams((cont_curr+1)/C, (energy_lim+1)/C, (motor_lim+1)/C, (motor_bias+1)/C)
